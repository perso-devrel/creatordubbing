...
> Upload one video and sub2tube dubs it into multiple languages, uploads the results to YouTube with captions, and surfaces watch-time analytics in a single dashboard.
...
- **Multi-audio track helper** — YouTube's multi-audio-track API is unavailable, so sub2tube organises per-language videos instead.
